...
JUNK_FILTER="(google-analytics|googletag|doubleclick|adsystem|facebook|fbcdn|gtag|tagmanager|hotjar|clarity|png|jpg|css|google|youtube|linkedin|svg|woff|woff2|ttf|otf|gif|ico)"

DOMAIN="myprotein.com"

# --- 1. مراقبة النطاقات الفرعية ---
print_status "Subdomain Discovery"
subfinder -d $DOMAIN -silent | anew all-domains.txt > new_domains.txt
...
# --- 3. استخراج ومراقبة ملفات JS (الحجم والتغير) ---
print_status "JS Monitoring (New Files & Size Changes)"

# استخراج روابط JS فقط من الروابط الحية
grep -Ei "\.js(\?|$)" all_live_urls.txt | grep -viE "$JUNK_FILTER" > current_js_list.txt

# استخدام httpx لجلب حجم الملف (Content-Length) لكل رابط JS
# النتيجة ستكون بصيغة: [https://url.com/file.js] [200] [12345]
httpx -l current_js_list.txt -status-code -content-length -silent > js_metadata_now.txt
...
if [ -s js_changes.txt ]; then
    echo "⚠️ JS Change Detected (New file or Size change):"
    cat js_changes.txt | notify -id js_discovery
fi

# تحديث قائمة النهاية للتنظيف
cat current_js_list.txt | anew FINAL_JS_ENDPOINTS.txt > /dev/null
